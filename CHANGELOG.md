...
- [MAJOR] `OpenApiSchemaGenerator` derives tag order from `tags` list position (removed `tagOrder` param), takes `tags: Sequence[OpenApiTag]` and `securitySchemes: Sequence[OpenApiSecurityScheme]` instead of raw dicts, and infers "documented" from `operationId` being set (removed `documented` param from `json_route`/`streaming_json_route`)
- [MINOR] Added `OpenApiSecurityScheme`; `authorize_bearer_jwt`/`authorize_signature`/`authorize_token` accept `securityScheme` and auto-publish route `security` metadata when applying auth
...
- [MAJOR] Added `RouteAuthResolver`; `route` accepts named auth policies plus a resolver and invokes it before rate limiting, replacing authorization decorator sequences.
