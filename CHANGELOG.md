...
### Added
- [MAJOR] Added injectable `RequestContextHolder` and `RequestContextMiddleware` for app-specific request state. `KibaApiRequest` carries `authJwt`, `authBasic`, and `originIp`; core authorization and rate limiting use those request fields. Removed global request-context accessors; app-specific context fields are configured through a holder and middleware factory.
- [MAJOR] `OpenApiSchemaGenerator` derives tag order from `tags` list position (removed `tagOrder` param), takes `tags: Sequence[OpenApiTag]` and `securitySchemes: Sequence[SecurityScheme]` instead of raw dicts, and infers "documented" from `operationId` being set (removed `documented` param from `json_route`/`streaming_json_route`)
- [MINOR] Added `SecurityScheme`; `authorize_bearer_jwt`/`authorize_signature`/`authorize_token` accept `securityScheme` and auto-publish route `security` metadata when applying auth
- [MAJOR] Added `core.api.route.route`, composing JSON transport (`json_route` or `streaming_json_route`), authorization decorators (`auth`, applied in declaration order), and `rate_limit` into one decorator (`route` applies `rate_limit(rateLimit)` directly to the handler before auth and before choosing JSON transport by `isStreaming` - no separate request-guard wrapper); `core.api.authorizer.authorize_token_request` centralizes token validation for `authorize_token`; `core.api.rate_limit.check_rate_limit` now takes a `RateLimitConfig` directly (builds windows and resolves `keyBy` internally per call, replacing the separate `build_rate_limit_windows` helper and `windows`/`keyBy` params); `RateLimitConfig` now carries `keyBy` (removed the separate `keyBy` param from `rate_limit`/`route`); `rate_limit` takes a single `RateLimitConfig` positional arg instead of `perMinute`/`perFiveMinutes`/`perHour`/`perDay`/`keyBy` kwargs
- [MAJOR] Added `RouteAuthResolver` and `create_route`; a bound `RouteBuilder` accepts named auth policies and invokes the resolver before rate limiting, replacing `route` authorization decorator sequences.
- [MAJOR] `json_route` and `streaming_json_route` now only parse requests and serialize responses; they no longer accept or publish OpenAPI metadata. Use `route`, or explicitly compose JSON transport with route metadata, for documented endpoints.
